...
Filters::Filters(Preferences* prefs, RadioInterface* radio_interface, int w, int h) : Fl_Window(w, h) {
	...
	for (i=0, j=0, k=0 ; i<m_b.n; i++) {
		...
		m_b.items[i].filter = new FilterButton(this, r_i, m_b.items[i].label, m_b.items[i].id, m, (Fl_Color)33, (Fl_Color)67);
		if (i == filter) {
...
//==============================================================================
// Mode buttons
FilterButton::FilterButton(Filters *top_level, RadioInterface* radio_interface, char* button_label, int mode_id, metrics m, Fl_Color back_col, Fl_Color label_col) : ToggleButtonBase(radio_interface, button_label, m.x, m.y, m.w, m.h, back_col, label_col) {
	t_l = top_level;
	r_i = radio_interface;
	id = mode_id;
}
